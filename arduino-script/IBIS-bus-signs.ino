...
void loop() {
    ...
    if(cycle_number != 0) { 
        if(updateIBIS_zM) {
            ...
            if(currect_InteriorSign_text_index == 0) {
                // length: 131
                // А Б В Г Д Е Є Ж З И І Ї Й К Л М Н О П Р С Т У Ф Х Ц Ч Ш Щ Ы Ь Ю Я а б в г д у є ж з и і ї й к л м н о п р с т у ф х ц ч ш щ ы ь ю я
                IBIS_zM(F("A B V G D E > H Z I @ W J K L M N O P R S T U F X C ^ < Q Y * $ ` a b v g d u ~ h z i { w j k l m n o p r s t u f x c } \\ q y ? # %"));
            } else if(currect_InteriorSign_text_index == 1) {
                // length: 254
                // Конотоп - місто в Сумській області України, адміністративний центр Конотопського району та Конотопської міської громади. Розташований на річці Єзуч, у північній частині України, на заході Сумщини за 131 км від обласного центру. Населення міста становить.
                IBIS_zM(F("Konotop - m{sto v Sums?k{j oblast{ Ukrawni, adm{n{strativnij centr Konotops?kogo rajonu ta Konotops?kow m{s?kow gromadi. Rozta\\ovanij na r{}c{ >zu}, u p{vn{}n{j }astin{ Ukrawni, na zaxod{ Sumqini za 131 km v{d oblasnogo centru. Naselenn% m{sta stanovit?."));
            } else if(currect_InteriorSign_text_index == 2) {
                // length: 227
                // Київ це столиця та найбільше місто України. Розташований у середній течії Дніпра, у північній Наддніпрянщині. Політичний, соціально-економічний, транспортний, освітньо-науковий, історичний, культурний та духовний центр України.
                IBIS_zM(F("Kiwv ce stolic% ta najb{l?\\e m{sto Ukrawni. Rozta\\ovanij u seredn{j te}{w Dn{pra, u p{vn{}n{j Naddn{pr%nqin{. Pol{ti}nij, soc{al?no-ekonom{}nij, transportnij, osv{tn?o-naukovij, {stori}nij, kul?turnij ta duxovnij centr Ukrawni."));
            }
